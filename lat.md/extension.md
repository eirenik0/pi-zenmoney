...
`/zen-profiles` opens the profile editor with separate live search filtering for profiles and accounts, hiding archived accounts by default and allowing new profile creation from the profile search; the balance command and tool both accept `--snapshot-path` / `snapshotPath`, and the value must stay relative to the working files folder.
