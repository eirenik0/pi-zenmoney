...
`/zenmoney` opens the settings hub for profiles, accounts, and snapshot-path setup; `/zen-profiles` remains a compatibility alias with the same profile-editing flow. Archived accounts stay hidden by default, and the balance command and tool both accept `--snapshot-path` / `snapshotPath`, which must stay relative to the working files folder.
