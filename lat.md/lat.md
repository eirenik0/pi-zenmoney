...
The `/zenmoney` hub opens a TUI for browsing profiles and current accounts with separate live search filters; archived accounts are hidden by default, typing a new name can create a profile, space toggles the highlighted account, and enter saves the chosen selectors back to the profile policy.
